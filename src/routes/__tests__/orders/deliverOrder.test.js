...
describe('Order Routes: Deliver an Order', () => {
    ...
    request(app)
      ...
      .end((err, res) => {
        ...
        db.Order.findOne({ where: { orderId: res.body.id } }).then((order) =>
          order
            .addMeal('46ced7aa-eed5-4462-b2c0-153f31589bdd', {
              through: { quantity: 1 },
            })
            .then(() =>
  ...
  it('should deliver order belonging to caterer', (done) => {
    db.Order.findOne({ where: { orderId: firstOrderId } }).then((order) =>
      order.update({ status: 'pending' }).then(() => {
        request(app)
          .post(`/orders/${firstOrderId}/deliver`)
          .set('Accept', 'application/json')
          .set('authorization', foodCircleToken)
          .send({ delivered: true })
          .end((err, res) => {
            expect(res.statusCode).toBe(200);
            expect(res.body.meals[0].delivered).toBe(true);
  ...
  it("should change caterer's order status to delivered when all his meals have been delivered", (done) => {
    request(app)
      .post(`/orders/${secondOrderId}/deliver`)
      .set('Accept', 'application/json')
      .set('authorization', foodCircleToken)
      .send({ delivered: true })
      .end((err, res) => {
  ...
  it('should return errors for invalid input', (done) => {
    request(app)
      ...
      .end((err, res) => {
        ...
        expect(res.body.errors.delivered.msg).toBe(
          'True and False are the only available options',
        );
  ...
  it("should return error when delivery status isn't specified", (done) => {
    request(app)
      ...
      .end((err, res) => {
        expect(res.statusCode).toBe(400);
        expect(res.body.errors.delivered.msg).toBe(
          'Delivery status must be specified',
        );
  ...
  invalidID({
    type: 'orderId',
    method: 'post',
    url: '/orders/efbbf4ad-c4ae-4134-928d-b5ee305ed5396478/deliver',
  ...
  notFound({
    method: 'post',
    url: '/orders/9ce447be-ee46-424e-82b8-ae4160e795b4/deliver',
    token: foodCircleToken,
    data: { delivered: true },
  });
  ...
  unAuthorized(
    'post',
    '/orders/e544248c-145c-4145-b165-239658857637/deliver',
  );
});
